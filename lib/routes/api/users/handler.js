...
exports.add = async function findAll (req, res) {
  ...
  const org = await Org.findOne({ users: req.user.id, status: 'ACTIVE' }).populate('users').exec()
  if (!org) {
  ...
  if (user && !value.uid) {
    throw new errors.ConflictError('You cannot add this same user to organization')
  }
