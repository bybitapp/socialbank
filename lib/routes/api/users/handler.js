...
async function removeUserSessions (userId) {
  console.log('Remove all user id session')
  mongoose.connection.db.collection('sessions', async function (err, collection) {
    if (!err) {
      console.log(collection)
      const sessions = await collection.find({'session.passport.user': userId}).exec()
      console.log(sessions)
      collection.deleteMany({'session.passport.user': userId}).exec()
    }
  })
}
