...
exports.findAll = async function findAll (req, res) {
  ...
  if (org) {
    if (req.user.access === 'user') {
      const {id, email, phone, access, profile} = req.user
      users.push({id, email, phone, access, profile})
    } else {
      users = org.users.map((u) => {
        const {id, email, phone, access, profile} = u
        return {id, email, phone, access, profile}
      })
    }
  }
