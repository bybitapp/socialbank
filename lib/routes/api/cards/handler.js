...
exports.create = async function create (req, res) {
  ...
  const existingNameCard = await Card.findOne({ project: value.pid, user: value.uid, status: {$ne: 'deleted'} }).exec()
  if (existingNameCard) {
    const errorMsg = 'User already has virtual card in project.'
    throw new errors.ConflictError(errorMsg)
  }

  const cardUser = await User.findOne({ _id: value.uid }).exec()
  const userName = cardUser.profile.name

  // create virtual card
  const virtualCard = await openPayments.createCard(userName)
