...
exports.updatePassword = async function (req, res) {
  ...
  const isNew = await isNewPassword(value.newPassword, user)
  if (isNew === false) {
...
exports.resetPassword = async function (req, res) {
  ...
  const isNew = await isNewPassword(value.password, user)
  if (isNew === false) {
    throw new errors.ConflictError('Please choose another password.')
  }
...
async function isNewPassword (passwordString, user) {
  const passwordHistory = user.passwordHistory || []
  const newPasswordHash = await user.genPasswordHash(passwordString)
  console.log(passwordHistory)
  console.log(newPasswordHash)
  return (
    passwordHistory.indexOf(newPasswordHash) === -1 &&
