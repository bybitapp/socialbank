...
exports.updatePassword = async function (req, res, next) {
  ...
  let user = await User.findOne({ _id: req.user.id, status: 'active' }).exec()

  if (user) {
    await user
      .comparePassword(updatePassword.oldPassword)
      .then(async (isMatch) => {
        if (isMatch) {
          ...
          console.log('errors', {msg: errorMsg})
          next(new errors.ConflictError(errorMsg))
        }
      })
      .catch(err => {
        next(err)
