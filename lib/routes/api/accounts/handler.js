...
const { sendPasswordReset, sendPasswordChanged } = require('lib/processors/email')
...
exports.updatePassword = async function (req, res) {
  const { error, reqBody } = schema.updatePassword(req.body)
  if (error) {
    logger.error('errors', error)
    throw new errors.ValidationError(error)
  }
  ...
  const isMatch = await user.comparePassword(reqBody.oldPassword)
  if (!isMatch) {
...
exports.forgotPassword = async function (req, res) {
  const { error, reqBody } = schema.forgotPassword(req.body)
  if (error) {
    logger.error('errors', error)
    throw new errors.ValidationError(error)
  }

  const user = await User.findOne({ email: reqBody.email, status: 'active' }).exec()
  if (!user) {
...
exports.resetPassword = async function (req, res) {
  const { error, reqBody } = schema.resetPassword(req.body)
  if (error) {
    logger.error('errors', error)
    throw new errors.ValidationError(error)
  }
