...
const {opc: { programmeKey, programmeId, username, password, ownerId, currency, country, issuingProvider, processingProvider }} = config
...
exports.createDeposit = async function createDeposit (value, extId, managedId) {
  ...
  const apiTest = new Opc.BankTransferSimulatorSpiApi()
  const requestTest = Opc.CreatePayinSimulationParams.constructFromObject({
    bankAmount: amount,
  ...
  const payin = await apiTest.bankTransferSimulatorSpiIdPayin(programmeKey, requestTest, opt)
